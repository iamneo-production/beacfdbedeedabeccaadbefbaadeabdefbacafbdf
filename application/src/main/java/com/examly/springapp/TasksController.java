...
public class TasksController {
	...
	public ResponseEntity<String> saveTask(@RequestBody Tasks tasks) 
	{
		tasksservice.saveTasks(tasks);
		return new ResponseEntity<>(tasks.getTaskId(), HttpStatus.OK);
	}
	@PutMapping("/changeStatus/{taskId}")
	public ResponseEntity<Integer> changeTaskStatus(@PathVariable("taskId") int taskId, @RequestParam("status") String status)
	{
